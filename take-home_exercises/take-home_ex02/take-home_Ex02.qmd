...
### 5.1.3 Top 5 Provinces with the highest number of drug cases over all the years
...
```{r}
wm_d111000_lw <- nb2listw(wm_d111000, style = 'B')
summary(wm_d111000_lw)
```

### 7.2 Adaptive Distance Weight Matrix
...
Here, we can see that all 75 provinces have exactly 8 links, and province like Phuket are not left behind.
...
Now, we join the Gi values to the thailand sf data frame
...
#### Plotting and Mapping Gi values with fixed distance weights
...
tmap_arrange(gi_total_cases, Gimap, asp=1, ncol=2)
```
...
#### Plotting and Mapping Gi values with adaptive distance weights
